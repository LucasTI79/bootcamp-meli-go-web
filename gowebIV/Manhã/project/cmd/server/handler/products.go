...
func (c *ProductHandler) GetAll() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if err != nil {
			...
			// 	"error": err.Error(),
			// })
      ctx.JSON(http.StatusBadRequest, web.NewResponse(int(http.StatusBadRequest), nil, "Erro interno do sistema"))
			return
		}

		if len(p) == 0 {
			// ctx.Status(http.StatusNoContent)
      ctx.JSON(http.StatusNotFound, web.NewResponse(404, nil, "Não há produtos armazenados !"))
			return
		}
